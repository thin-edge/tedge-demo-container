...
configure_users() {
    ...
    if [ ! -f /etc/sudoers.d/tedge ]; then
        sudo sh -c "echo 'tedge  ALL = (ALL) NOPASSWD:SETENV: /usr/bin/tedge, /usr/bin/tedge-write /etc/*, /etc/tedge/sm-plugins/[a-zA-Z0-9]*, /usr/share/tedge/log-plugins/[a-zA-Z0-9]*, /usr/share/tedge/config-plugins/[a-zA-Z0-9]*, /bin/sync, /sbin/init, /bin/systemctl, /bin/journalctl, /sbin/shutdown, /usr/bin/on_shutdown.sh, /usr/bin/tedge-container' > /etc/sudoers.d/tedge"
    fi
}
